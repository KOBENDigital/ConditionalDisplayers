...
$params = '..\src\ConditionalDisplayers\ConditionalDisplayers.sln', '-property:Configuration=Release', '-verbosity:minimal', '-p:PostBuildEventUseInBuild=false'

& $msBuild $params

# Create Nuget package
.\nuget.exe pack -Build ..\src\ConditionalDisplayers\ConditionalDisplayers.csproj
Write-Host "Nuget package created."

# Create Umbraco package
Unblock-File -Path .\CreateUmbracoPackage.ps1
powershell -File .\CreateUmbracoPackage.ps1 -packageId Our.Umbraco.ConditionalDisplayers -packageDirectory ..\src\ConditionalDisplayers -buildConfiguration Release
